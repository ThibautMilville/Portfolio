...
export default function ProjectCarousel({ projects, isFeatured = false }: ProjectCarouselProps) {
  ...
  useEffect(() => {
    ...
    if (!project) return;
    const imgs = (project.screenshots && project.screenshots.length > 0) ? project.screenshots : [project.image];
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowRight') {
        ...
        setImageIndexByProject(prev => {
          const current = prev[openProjectId] ?? 0;
          return { ...prev, [openProjectId]: (current - 1 + imgs.length) % imgs.length };
        });
      }
    };
        ...
        >
          {projects.map((project, index) => (
            ...
            >
                ...
                <Link href={`/projets/${getProjectSlug(project as any)}`}>
                  ...
                  <div className={`relative w-full overflow-hidden ${isFeatured ? 'h-80' : 'h-48'}`}>
                    ...
                    <div className="absolute top-4 right-4 z-10">
                      <Badge variant="secondary" className="text-xs px-3 py-1 rounded-full bg-white/90 text-primary dark:bg-black dark:text-white border-0 font-semibold shadow">
                        {project.category}
                      </Badge>
                    </div>
              ...
              <Dialog onOpenChange={(open)=>{ if (open) { setImageIndexByProject((prev)=>({ ...prev, [project.id]: 0 })); setOpenProjectId(project.id); } else { setOpenProjectId(prev => prev === project.id ? null : prev); } }}>
                ...
                <DialogContent className="w-[96vw] sm:w-auto max-w-3xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
                  <DialogHeader>
                    ...
                    <DialogDescription className="text-muted-foreground">{project.category} • {project.status}</DialogDescription>
                  </DialogHeader>
                  ...
                  <div className="space-y-6">
                    ...
                    {(() => {
                      const imgs = (project.screenshots && project.screenshots.length > 0) ? project.screenshots : [project.image];
                      const activeIndex = imageIndexByProject[project.id] ?? 0;
                      const goPrev = () => setImageIndexByProject((prev)=>({ ...prev, [project.id]: (activeIndex - 1 + imgs.length) % imgs.length }));
                      const goNext = () => setImageIndexByProject((prev)=>({ ...prev, [project.id]: (activeIndex + 1) % imgs.length }));
                      return (
                        <div className="space-y-3">
                          <div className="relative w-full h-[48vh] sm:h-64 overflow-hidden rounded-xl bg-black">
                            <img src={imgs[activeIndex]} alt={`${project.title} - Vue ${activeIndex+1}`} className="w-full h-full object-cover block select-none pointer-events-none" draggable={false} />
                            {imgs.length > 1 ? (
                              <>
                                <button
                                ...
                                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
                                  {imgs.map((_, i) => (
                                    <button
                                      key={i}
                                      onClick={(e)=>{ e.preventDefault(); e.stopPropagation(); setImageIndexByProject((prev)=>({ ...prev, [project.id]: i })); }}
                                      className={`w-2 h-2 rounded-full transition-all focus:outline-none focus-visible:outline-none ring-0 focus:ring-0 ${i === activeIndex ? 'bg-white scale-125' : 'bg-white/50 hover:bg-white/75'}`}
                            ...
                            <div className="grid grid-cols-5 gap-2">
                              {imgs.map((src, i) => (
                                <button
                                  key={src + i}
                                  onClick={(e)=>{ e.preventDefault(); e.stopPropagation(); setImageIndexByProject((prev)=>({ ...prev, [project.id]: i })); }}
                                  className={`relative rounded-md overflow-hidden border focus:outline-none focus-visible:outline-none ring-0 focus:ring-0 ${i === activeIndex ? 'border-primary' : 'border-transparent'}`}
                                  aria-label={`Miniature ${i+1}`}
                                >
                                  <img src={src} alt={`${project.title} miniature ${i+1}`} className="w-full h-16 object-cover" />
                                </button>
                              ))}
                            </div>
                          ) : null}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {project.technologies.map((tech, i) => (
                            <Badge key={tech + i} variant="outline" className="rounded-full font-medium bg-gradient-to-r from-primary/10 to-secondary/10 text-primary border-primary/20 text-xs px-2 py-1">
                              {tech}
                            </Badge>
                          ))}
                        </div>
