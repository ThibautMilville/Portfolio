      ...
      <DialogTrigger asChild>
        ...
        >
          <img
            src={skill.logo}
            alt={`Logo ${skill.name}`}
            className="mb-2 h-11 w-11 rounded bg-white p-1 shadow-sm"
          />
...
export default function SkillsSection() {
  ...
  const skillCategories = [
    ...
    },
    {
      ...
      key: "Infrastructure and deployment",
    ...
    <section className="relative px-6 py-6 md:py-8">
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        <motion.div
          ...
          className="mb-16"
        >
          <SectionHeading title={t("title")} subtitle={t("subtitle")} icon="skills" />
        </motion.div>
            ...
            {categoriesWithSkills.map((category, index) => {
              return (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="group relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-background/90 to-background/70 p-5 backdrop-blur-sm md:p-6"
                >
                  <div
                    className={`pointer-events-none absolute inset-0 bg-gradient-to-r ${category.color} opacity-60 blur-3xl`}
                  />
        ...
        >
          <SectionHeading
            title={t("favoriteStack.title")}
            subtitle={t("favoriteStack.subtitle")}
            icon="stack"
            className="mb-8"
          />
